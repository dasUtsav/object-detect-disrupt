...
class CarliniWagner(object):
    ...
    def _optimize(self, model, optimizer, modifier, inputs, labels, scale_const):
        ...
        loss1 = torch.clamp(real - other + self.confidence,
    ...
    def attack(self, inputs, labels, model, *args):
        ...
        input_vars = (inputs.data - self.clip_min) / \
        ...
        input_vars = (torch.log((1 + input_vars) /
                                (1 - input_vars))) * 0.5  # arctanh
        ...
        modifier_var = Variable(
            modifier.cuda() if self.cuda else modifier, requires_grad=True)
        ...
        for outer_step in range(self.binary_search_steps):
            ...
            # .float() needed to conver to FloatTensor
            scale_const_tensor = torch.from_numpy(scale_const).float()
            scale_const_var = Variable(scale_const_tensor.cuda(
            ) if self.cuda else scale_const_tensor, requires_grad=False)
            ...
            for i in range(batch_size):
                if self._compare(best_score[i], labels[i]) and best_score[i] != -1:
                    ...
                    upper_bound[i] = (lower_bound[i] + upper_bound[i]) / \
                        2 if (upper_bound[i] < 1e9) else (scale_const[i] * 10)
            ...
            if self.verbose:
                print("failures: {0} successes: {1}".format(
...
class DCGAN(object):
    ...
    def attack(self, inputs, labels, model, model_optimizer=None, epsilon=1.0, *args):
        ...
        loss = torch.exp(-1 * self.criterion(predictions, labels)
                         ) + self.cg * (torch.norm(perturbation, 4))
